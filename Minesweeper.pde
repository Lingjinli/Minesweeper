...
public void setBombs()
{
    ...
    {
        int ling = (int)(Math.random()*NUM_ROWS);
        int li = (int)(Math.random()*NUM_COLS);
            if(!(bombs.contains(buttons[ling][li])))
            {
                bombs.add(buttons[ling][li]);
            }
    }
}
...
{
    String winmessage = "Congratz";
    for(int u = 1; u < winmessage.length()+1; u++)
    {
